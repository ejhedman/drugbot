-- Updated: 2025-07-04 - Improved filter handling for empty arrays and single-value quoting
...
-- Function to get distinct rows with filtering and paging
...
BEGIN
  ...
  RAISE NOTICE 'PARAMS: table_name=%, column_list=%, filters=%, page_offset=%, page_limit=%, order_by=%', table_name, column_list, filters, page_offset, page_limit, order_by;
  -- Validate inputs
  ...
  IF filter_keys IS NOT NULL THEN
    FOR i IN 1..array_length(filter_keys, 1) LOOP
      ...
      ELSE
        -- Single value - use equality, strip quotes
        filter_conditions := filter_conditions || format('%I = %L', filter_key, trim(both '"' from filter_value::text));
      END IF;
    END LOOP;
  END IF;
  ...
  -- Execute the query and log the number of rows returned
  RETURN QUERY EXECUTE query_text;
...
GRANT EXECUTE ON FUNCTION get_distinct_rows_with_filters(text, text[], jsonb, integer, integer, text) TO authenticated; 